...
def detect_id_card_in_frame(frame):
    ...
    for id_card in id_cards.boxes.data.tolist():
        ...
        # Apply EasyOCR to the cropped ID card image
        reader = easyocr.Reader(['en'])  # English language
        result = reader.readtext(id_card_crop)

        # Extract and print the OCR result
        if result:
            ocr_text = result[0][1]  # Extracting text from the result
